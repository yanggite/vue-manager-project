<template>
    <div class="guidePage">
        <div class="main">
            ...
            <el-card>
                <el-table :data="tableData" style="width: 100%" border>
                    ...
                    <el-table-column  label="缩略图" align="center" header-align="center">
                        <template slot-scope="scope">
                            <el-image style="max-width: 100px; max-height: 100px" :src="scope.row.imgList[0]"></el-image>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="发布时间" align="center" header-align="center">
                        <template slot-scope="scope">
                            <span v-if="!scope.row.creatTime">无</span>
                            <span v-else>{{ scope.row.creatTime | dateYMDHMSFormat }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" align="center" header-align="center" width="200">
                        <template slot-scope="scope">
                            ...
                            <el-link  type="danger"  @click="onDelect(scope.row)" v-if="btnList.del"
                                >删除</el-link
                            >
                        </template>
                    </el-table-column>
        ...
        <AddGuidePage :addGuideVisible.sync="addGuideVisible" v-if="addGuideVisible"  @refreshList="refreshList"/>
        <EditGuidePage :editGuideVisible.sync="editGuideVisible" v-if="editGuideVisible"  @refreshList="refreshList" :currentItem="currentItem"/>
    </div>
</template>

<script>
import { mixins } from '@/utils/mixins';
import AddGuidePage from './component/addGuidePage';
import EditGuidePage from './component/editGuidePage'
export default {
    name: 'guidePage',
    mixins: [mixins],
    components: { AddGuidePage,EditGuidePage },
    data() {
        return {
            addGuideVisible: false,
    ...
    methods: {
        async getListInit() {
            const res = await this.$api.getGuidePageList({ pageNo: this.pageNo, pageSize: this.pageSize });
            if (res.code === '200') {
        ...
        async onDelect(item) {
            ...
            if (result === 'confirm') {
                //    console.log(item)
                const data = await this.$api.delGuidePage({ id: item.id });
