        ...
        <el-card>
            <el-form :model="form" :rules="rules" ref="form" :label-position="labelPosition" label-width="110px">
                ...
                <el-form-item label="会员价格" prop="isLeaveValue">
                    <el-radio-group v-model="form.isLeaveValue" @change="isLeaveValueChange">
                        <el-radio :label="0">否</el-radio>
                        <el-radio :label="1">是</el-radio>
                    </el-radio-group>
                </el-form-item>

                <div class="vipLevel" v-show="form.isLeaveValue == '1'">
                    <el-form-item prop="leaveValueList">
                        <table>
                            <tr>
                                <th v-for="item in form.leaveValueList" :key="item.id" :dataset="item.id">{{ item.leaveName }}</th>
                            </tr>
                            <tr>
                                <td v-for="item in form.leaveValueList" :key="item.id" :dataset="item.id">
                ...
                <el-form-item label="返利拨付总额" prop="pi">
                    <!-- <el-input v-model="form.pi" autocomplete="off" class="inputPure" placeholder="输入日拨付比例（0-1）"></el-input> -->
                    <el-input
                        v-model="form.pi"
                        type="number"
                ...
                <el-form-item label="商品图片" v-if="editStatus">
                    <div>
                        ...
                        <el-image style="width:150px;height:150px" :src="form.imgList[0]" alt="" :preview-src-list="form.imgList" />
                    </div>
                </el-form-item>
                <el-form-item label="重传图片" prop="imgList">
                    <div>
                        <el-upload
                            multiple
                            :limit="3"
...
export default {
    data() {
        return {
            form: {
                ...
                isUp: '', // 商品上架     0.否，1.是
                isSeckill: '', // 秒杀商品       0.否，1.是
                seckillStartTime: '', //秒杀开始时间
                seckillEndTime: '', // 秒杀结束时间
                limitNum: '', // 限购数量
                seckillNum: '', // 秒杀价
                isLeaveValue: '', // 是否有会员价  0.否，1.是
                leaveValueList: [
                    {
                        id: '',
                        leaveId: '',
    ...
    methods: {
        async getShopBaseInfoById(id) {
            const {
                data: { imgList, shopBaseInfo, shopDetail, isLeaveValue, isSeckill, shopLeaveValues, language }
            } = await this.$api.getShopBaseInfoById({ id });
            // console.log(imgList);
            this.form = { ...shopDetail, isLeaveValue, isSeckill, language, ...shopBaseInfo };
            this.$set(this.form, 'leaveValueList', shopLeaveValues); //响应式
            this.form.imgList = imgList;
            console.log(this.form);
            await this.getSelectLanguage();
            await this.getSelectProductCategory(language);
            if (shopLeaveValues.length == 0) {
                this.isGetVipLevel = true;
            }
        },
        async getShopSelectLeave() {
            const res = await this.$api.getShopSelectLeave();
            console.log(res);
            if (res.code === '200') {
        ...
        async languageChange(value) {
            this.getSelectProductCategory(value);
        },
        async getSelectProductCategory(value) {
            if (value) {
        ...
        onSumbit() {
            this.$refs.form.validate(async valid => {
                if (valid) {
                    // if(this.form.isLeaveValue=='0'){
                    //     for(let index in this.form){
                    //         // console.log(index)
                    //         if(index=='leaveValueList'){
                    //             console.log(this.form[index])
                    //             delete this.form[index]
                    //         }
                    //     }
                    // }
                    const res = await this.$api.editCommodity(this.form);
                    if (res.code === '200') {
        ...
        saleChange() {},
        isLeaveValueChange() {
            if (this.isGetVipLevel && this.form.isLeaveValue == 1) {
