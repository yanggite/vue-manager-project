<template>
    <div class="updateDeliveryState dialogSingle">
        <el-dialog title="修改发货状态" :visible.sync="CVisible" class="dialogueV">
...
export default {
    ...
    computed: {
        CVisible: {
            get() {
                return this.deliveryStateVisible;
            },
            set(val) {
                this.$emit('update:deliveryStateVisible', val);
            }
        }
    },
    data() {
    ...
    created() {
        ...
        this.form.state = this.currentItem.deliveryStatus;
        this.memberOption.forEach((item, index) => {
            if (item.val == this.currentItem.deliveryStatus) {
    ...
    methods: {
        onCancle() {
            // this.$emit('update:deliveryStateVisible', false);
            this.CVisible = false;
        },
        onSumbit() {
            this.$refs.form.validate(async valid => {
                if (valid) {
                       const res = await this.$api.editOrderDeliveryStatus(this.form);
                    if (res.code === '200') {
